...
export default function EditTransactions(props) {
    ...
    const [transactionIdLabel, setTransactionIdLabel] = useState({
        id: '',
        label: '',
    });

    const { mutateAsync: updateTransaction, isLoading } = useMutation(
        EmployeeService.updateTransaction,
        {
            onSuccess: async ({ data }) => {
                if (data?.errors?.status !== 400) {
                    await props.transactionRefetch();
                    successNotify('Transaction successfully updated!');
    ...
    const { data: transaction } = useQuery('getTransactionById', () =>
        EmployeeService.getTransactionById(props.transactionId)
    );
    ...
    useEffect(() => {
        if (transaction) {
            ...
            setExternalId(transaction?.externalId);
            setPaymentSystem(transaction?.paymentSystem);
            setEmployeeId(transaction?.employeeId);
            setAmount(transaction?.amount);
            setSelectedEmployee(
                employee.find((el) => el.id === transaction?.employeeId)
            );
            setTransactionIdLabel(() => {
                const transactionLabel = employee.find(
                    (el) => el._id === transaction.employeeId
                );
                return {
                    label: `${transactionLabel?.firstName} ${transactionLabel?.lastName} - ${transactionLabel?.company?.companyName}`,
                ...
                <Grid container spacing={2}>
                    <Grid item xs={12} style={{ marginTop: 20 }}>
                        <Stack direction="row" justifyContent="end">
                            <Button
                                onClick={props.handleCloseTransactionModal}
                                variant="outlined"
                                style={{
                                    marginTop: 20,
                                    marginRight: 10,
